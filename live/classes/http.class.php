...
class wsLiveHttpRequest {
	...
	var $noErrors=true; // whether to trigger an error in case of a curl error
	var $errorMessage;
	var $debug=false;
	var $forceWithRedir=array();
	...
	function connect($url, $withHeaders=true, $withCookies=true) {
		...
		if ($this->debug) {
			echo '<br />call:' . $url;
			echo '<br />post=' . print_r($this->post, true) . '=<br />';
		}		
		...
		$cookies="";
		//print_r($_SESSION[$this->sid]['cookie-array']);
		...
		if ($cookies) {
			//echo 'Cookie before:' . print_r($cookies, true);
		...
		if ($cookies) {
			//echo 'Cookie after:' . print_r($cookies, true);
		...
		//if (is_array($cookies)) echo 'Cookie after:' . print_r($cookies, true);
		...
		if ($this->follow && isset($headers['location']) && $headers['location']) {
			...
			if ($this->debug) echo '<br />redir1=' . $redir;
			...
			if (strstr($redir, '&')) $and='&';
			elseif (strstr($redir, '?')) $and='&';
			else $and='?';
			// $redir.='wpabspath=0';
			if (count($this->forceWithRedir) > 0) {
				foreach ($this->forceWithRedir as $n => $v) {
					if (!strstr($redir, $n . '=' . $v)) $redir.=$and . $n . '=' . $v;
					$and='&';
				}
			}
